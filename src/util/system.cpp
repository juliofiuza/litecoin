...
const char * const BITCOIN_CONF_FILENAME = "emolumento.conf";
...
#else
    const char* pszModule = "emolumento";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Emolumento";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Emolumento";
#else
    // Unix
    return pathRet / ".emolumento";
#endif
#endif
}
